...
schroot_output = os.path.join(os.path.dirname(__file__), "data", "schroot_output.txt")
_chroot_parameters = [
    {
...
class TestSbuildDriver(base.TestCase):
    ...
    @mock.patch.object(SbuildDriver, "sys_execute")
    def test_sys_execute_command(self, m_execute):
    ...
    def test_add_apt_sources_except_listd_dir(self, m_isdir):
        ...
        with self.assertRaisesRegex(
                ValueError,
                """There is no apt sources.list.d in /fake/path/etc/apt/sources.list.d/"""):
            self.driver.add_apt_sources(
