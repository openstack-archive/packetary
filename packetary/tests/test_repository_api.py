...
class TestRepositoryApi(base.TestCase):
    ...
    def test_validate_invalid_data(self, jsonschema_mock):
        with self.assertRaisesRegexp(
                ...
                "Invalid input for field/attribute 1. Value 1."):

            invalid_data = [1, 456]
            jsonschema_mock.validate.side_effect = jsonschema.ValidationError(
                "Invalid input for field/attribute 1. Value 1."
            )
            self.api._validate_data(invalid_data, PACKAGES_SCHEMA)

        with self.assertRaisesRegexp(jsonschema.ValidationError,
                                     "{} is not o type 'array'"):
