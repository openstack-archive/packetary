...
class TestMockDriver(base.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        MockDriver.get_chroots = lambda x: mock_releases
        MockDriver.find_system_mock = lambda x: "/usr/bin/mock"
        super(TestMockDriver, cls).setUpClass()
        cls.sys_execute = mock.MagicMock()
        cls.driver = MockDriver()
    ...
    def test_render_repo_config_open_file(self, tempfile_m):
        tempfile_m.return_value = "/tmp/file"
        m = mock.mock_open()
        with mock.patch("__builtin__.open", m, create=True):
            self.driver.render_repo_config([{"name": "fake name"}])

        handler = m()
        handler.write.assert_has_calls([
            mock.call("[fake name]\n"),
            mock.call("name = fake name\n"),
            mock.call("\n")
        ])
