...
class RepositoryApi(object):
    ...
    def _load_filters(self, filters_data):
        ...
        - name: full-package-name
          section: section1
        - name: /^.*substr/
        Value could be a string or a python regexp.
        ...
        for fdata in filters_data:
            ...
            for key, value in six.iteritems(fdata):
                if value.startswith('/') and value.endswith('/'):
                    ...
                    matchprops.append(lambda p: regex.match(getattr(p, key)))
                else:
                    matchprops.append(lambda p: getattr(p, key) == value)
            filters.append(lambda p: all((m(p) for m in matchprops)))
        return filters
