...
class MockDriver(PackagingDriverBase):
    def __init__(self):
        ...
        self.srpm_template = '%s -r %s --resultdir %s --buildsrpm ' \
                             '--sources %s --spec %s'
        self.rpm_template = '%s -r %s --resultdir %s --rebuild %s'
    ...
    def get_chroots(path_to_configs='/etc/mock/'):
        ...
        if not os.path.exists(path_to_configs):
            raise ValueError(
                'There is no chroot configs path %' % path_to_configs
            )
