...
class MockDriver(PackagingDriverBase):
    def __init__(self):
        ...
        self.srpm_templeate = '{0} -r {1} --resultdir {2} --buildsrpm ' \
                              '--sources {3} --spec {4}'
        self.rpm_templeate = '{0} -r {1} --resultdir {2} --rebuild {3}'

    def check_release(self, release):
        if release not in self.releases:
            raise ValueError('There is no "{0}" in mock chroot configs, '
    ...
    def build_srpm(self, release, sources, spec=None, resultdir=None):
        ...
        command = self.srpm_templeate.format(
            self.mock_bin,
            release,
            resultdir,
            sources,
    ...
    def build_rpm(self, release, srpms, resultdir=None):
        ...
        for srpm in validated_srpms:
            command = self.rpm_templeate.format(
                self.mock_bin,
                release,
                resultdir,
                srpm
