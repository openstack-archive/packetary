...
class MockDriver(PackagingDriverBase):
    ...
    @staticmethod
    def render_repo_config(repos):
        """Rendering yum config for build chroot

        :param repos: list of repos with parameters
        :type repos: list

        :return: path to yum config file
        :rtype str
        """
        repo_file = tempfile.mkstemp(prefix='packetary_repo_')[1]
        config = ConfigParser.ConfigParser()
        for repo in repos:
            name = repo.get('name', 'NoName')
        ...
        with open(repo_file, 'w') as config_file:
            config.write(config_file)

        return repo_file

    @staticmethod
    def append_mock_config(maincfg, appendcfg):
        """Append repo config to mock-build cfg file

        :param maincfg: main mock-build config file path
        :type maincfg: str

        :param appendcfg: appending repo config file
        :type appendcfg: str

        :return None
        :rtype None
        """
        append_templ = """config_opts['yum.conf'] += open("{0}").read()"""
        config = []

        if not os.path.isfile(maincfg):
            raise ValueError("File {0} doesn't exists".format(maincfg))

        if not os.path.isfile(appendcfg):
            raise ValueError("File {0} doesn't exists".format(appendcfg))

        with open(maincfg, 'r') as config_read:
            config = config_read.readlines()

        if config[-1].startswith(append_templ[:30]):
            config = config[:-1]

        config.append(append_templ.format(
            os.path.abspath(appendcfg)
            )
        )

        with open(maincfg, 'w') as config_write:
            for line in config:
    ...
    def build_packages(self,
        ...
        if repos:
            yum_repo_config = self.render_repo_config(repos)
            self.append_mock_config(
                '/etc/mock/{0}.cfg'.format(release),
                yum_repo_config
            )
        srpms = self.build_srpm(release, sources, spec_file, resultdir)
        rpm_result = self.build_rpm(release, srpms['result'], resultdir)
