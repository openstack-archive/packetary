...
class MockDriver(PackagingDriverBase):
    ...
    def check_release(self, release):
        if release in self.releases:
            ...
            raise ValueError('There is no "%s" in mock chroot configs, '
    ...
    def get_chroots(path_to_configs='/etc/mock/'):
        ...
        if not os.path.exists(path_to_configs):
            raise ValueError('There is no chroot configs path %' % path_to_configs)

        chroots = []
        for filename in glob.iglob(os.path.join(path_to_configs, '*.cfg')):
    ...
    def build_srpm(self, release, sources, spec=None, resultdir=None):
        ...
        if not os.path.exists(sources):
            raise ValueError('Sources exists "%s" ?' % sources)

        if not spec:
            available_specs = list(glob.iglob(os.path.join(sources, '*.spec')))

            if not available_specs:
                raise ValueError('There is no spec file in sources, please specify it')
            else:
        ...
        srpms = list(glob.iglob(os.path.join(resultdir, '*.src.rpm')))
    ...
    def build_rpm(self, release, srpms, resultdir=None):
        ...
        result['rpms'] = list(glob.iglob(os.path.join(resultdir, '*.rpm')))
